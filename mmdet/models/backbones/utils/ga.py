...
def mutation(offspring_crossover, num_mutations=1):
    ...
    for l in range(max_layers):
        ...
        random_value = np.random.randint(low=0, high=12)
        if mut_pro == 1:
            offspring_crossover[0, l] = random_value
        
    return offspring_crossover

def calculate_macs(layers_info, candidate_table):
    max_layers = len(layers_info)
    macs = 0

    for l in range(max_layers):
        macs += candidate_table[l]["macs"][layers_info[l]]
